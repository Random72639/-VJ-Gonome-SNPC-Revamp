...
ENT.MeleeAttackBleedEnemyDamage = 5 -- How much damage per repetition
ENT.MeleeAttackBleedEnemyTime = 1 -- How much time until the next repetition?
ENT.MeleeAttackBleedEnemyReps = 7 -- How many repetitions?
...
self.MeleeAttackDamageType = bit.bor(DMG_SLASH,DMG_POISON)
...
ENT.AssassinHCThrowT = 0
ENT.HCThrown = false

function ENT:CustomOnThink_AIEnabled()	
if IsValid(self:GetEnemy()) && CurTime() > self.AssassinHCThrowT && self:IsOnGround() && self:GetPos():Distance(self:GetEnemy():GetPos()) < 699  then
if !IsValid(self.Headcrab1) && !self.HCThrown then
end
...
Headcrab1:SetPos(self:GetPos() +self:GetForward()*100 +self:GetUp()*5)
Headcrab1:SetAngles(self:GetAngles() +Angle(0,math.random(0,360),0))
Headcrab1:Spawn()
Headcrab1:SetNoDraw(true)
Headcrab1:SetVelocity(self:GetForward() * 700 +self:GetUp()*260)
Headcrab1:VJ_DoSetEnemy(ene,true)		
timer.Simple(0.3,function() if IsValid(Headcrab1) then Headcrab1:SetNoDraw(false) end end)
self.Headcrab1 = Headcrab1
self.HCThrown = false
self.AssassinHCThrowT = CurTime() + 30
end
end
---------------------------------------------------------------------------------------------------------------------------------------------
	-- ====== Sound File Paths ====== --
